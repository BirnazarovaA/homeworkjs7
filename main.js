...
const deleteProduct = async (id) => {await fetch(`${endpoints.products}${id}`, {method: 'DELETE'});
    await getAllProducts();
}

const getProduct = async (id) => {const res = await fetch(`${endpoints.products}${id}`, {method: 'GET'})
 ...
 submit.onclick =  () => {
  const obj = {
   ...
   fetch(`${endpoints.products}$[id}`,{method:'PUT', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(obj)})}
 await getAllProducts()
}

const getAllProducts = async () => {const products = document.querySelector('.products');
 products.innerHTML = "";
...
const addProduct = async () => {
 ...
 await fetch(endpoints.products, {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(obj)})
 await getAllProducts();
}
